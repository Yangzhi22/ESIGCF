...
class SimGCL(nn.Module):
    ...
    def aggregate(self, perturbed=False):
        ...
        # no dropout
        # Initial embedding is not included in the official implementation of SimGCL
        embeddings = []
    ...
    def forward(self, user, positive, negative):
        ...
        user_embedding_1, item_embedding_1 = self.aggregate(perturbed=True)
        user_embedding_2, item_embedding_2 = self.aggregate(perturbed=True)
...
class Trainer():
    def __init__(self, args, config, dataset, device, logger):
        self.model = SimGCL(config, dataset, device)
        self.args = args
        self.device = device
        self.config = config
        self.dataset = dataset
