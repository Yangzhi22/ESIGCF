"""
PyTorch Implementation of LR-GCCF
Revisiting Graph based Collaborative Filtering: A Linear Residual Graph Convolutional Network Approach  Chen et al. AAAI'20
"""
...
class GCCF(nn.Module):
    ...
    def aggregate(self):
        ...
        for layer in range(int(self.config['GCN_layer'])):
            ...
            # (L+I)·E·W1
            ego_embeddings = torch.matmul(side_embeddings, self.weight_dict['W_gcn_%d' % layer]) \
...
class Trainer():
    def __init__(self, args, config, dataset, device, logger):
        self.model = GCCF(config, dataset, device)
        self.args = args
        self.device = device
        self.config = config
        self.dataset = dataset
