...
########################################################################################################################
def joint_sparse_adjacency_matrix(data):
    try:
        norm_adjacency = sp.load_npz(data.path + '/pre_A1.npz')
        print("\t Adjacency matrix1 loading completed.")
    except:
        adjacency_matrix = sp.dok_matrix((data.num_nodes, data.num_nodes), dtype=np.float32)
        adjacency_matrix = adjacency_matrix.tolil()
        R = data.user_item_net.todok()
        ...
        ################################################################################################################
        d_inv1 = np.power(row_sum, -1).flatten()
        d_inv1[np.isinf(d_inv1)] = 0.
        degree_matrix1 = sp.diags(d_inv1)
        ################################################################################################################

        norm_adjacency = degree_matrix.dot(adjacency_matrix).dot(degree_matrix).tocsr() + degree_matrix1.dot(adjacency_matrix).tocsr()
        sp.save_npz(data.path + '/pre_A1', norm_adjacency)
        print("\t Adjacency matrix1 constructed.")

    return norm_adjacency


def joint_sparse_adjacency_matrix_R(data):
    try:
        norm_adjacency = sp.load_npz(data.path + '/pre_R1.npz')
        print("\t User–item interaction matrix1 loading completed.")
    except:
        adjacency_matrix = data.user_item_net
        ...
        ################################################################################################################
        row_d_inv1 = np.power(row_sum, -1).flatten()
        row_d_inv1[np.isinf(row_d_inv1)] = 0.
        row_degree_matrix1 = sp.diags(row_d_inv1)
        ################################################################################################################

        norm_adjacency = row_degree_matrix.dot(adjacency_matrix).dot(col_degree_matrix).tocsr() + row_degree_matrix1.dot(adjacency_matrix).tocsr()
        sp.save_npz(data.path + '/pre_R1', norm_adjacency)
        print("\t User–item interaction matrix1 constructed.")
